...
class FT4GameDBRowInstance;
class T4EDITORCOMMON_API ST4TextDBTreeWidget : public ST4TreeViewWidget
{
public:
	SLATE_BEGIN_ARGS(ST4TextDBTreeWidget) {}
	...
	void Construct(const FArguments& InArgs, const FT4TextDBTreeColumnWidthInfo& InColumnWidthInfo);
	...
	void SetColumnManualWidth(const FT4TextDBTreeColumnWidthInfo& InTreeColumnWidthInfo); // #158
	void GetColumnManualWidth(FT4TextDBTreeColumnWidthInfo& OutTreeColumnWidthInfo); // #158
...
protected:
	...
	void GetColumnData(const FT4GameDBRowInstance* InDBRowInstance, FT4TreeViewNodePtr OutItem);
	TSharedPtr<SHeaderRow> GenerateHeaderWidget() override;

	void HandleOnCopyToClipboard(); // #172
	bool HandleOnCanCopyToClipboard(); // #172
...
private:
	...
	FT4TextDBTreeColumnWidthInfo TreeColumnWidthInfo; // #158
};
